...
public class VarIsNotUsed implements VisitanteASA, Dica {
    ...
    public List<Mensagem> recuperar() {
        ...
        for (final Simbolo s : memoria.values()){
            if (!s.utilizado()){
                if (!s.getNome().equals(funcaoPrincipal)){
                    dicas.add(new Mensagem() {
                        
                        @Override
                        protected String construirMensagem() {
                           return "O simbolo "+ s.getNome() + " não foi utilizado.";
                        }
                    });                
                }
            }
