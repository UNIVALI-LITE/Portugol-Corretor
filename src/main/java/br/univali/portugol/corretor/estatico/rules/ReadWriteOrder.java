...
public class ReadWriteOrder implements VisitanteASA, Dica {
    ...
    private List<Simbolo> entrada = new ArrayList<Simbolo>();
    private List<Simbolo> saida = new ArrayList<Simbolo>();
    
    @Override
    public List<Mensagem> recuperar() {
        List<Mensagem> dicas =  new ArrayList<Mensagem>();
        for (int i = 0; i < entrada.size(); i++){
            System.out.println(entrada.get(i) + " != " +  saida.get(i));
            if (entrada.get(i) != saida.get(i)){
                dicas.add(new Mensagem() {

                    @Override
                    protected String construirMensagem() {
                        return "As variáveis utilizadas durante a saída devem estar na mesma ordem de leitura.";
                    }
                });
            }
        }
    ...
    public Object visitar(NoChamadaFuncao chamadaFuncao) throws ExcecaoVisitaASA {
        ...
        if (!"escreva".equals(chamadaFuncao.getNome())
            ...
            for (NoExpressao noExpressao : parametros) {
                ...
                Object expressao = noExpressao.aceitar(this);
                
                if (expressao instanceof Simbolo) {
                    Simbolo s = (Simbolo) expressao;
                    if (s != null){
    ...
    public Object visitar(NoPara noPara) throws ExcecaoVisitaASA {
        final NoBloco inicializacao = noPara.getInicializacoes().get(0);
